...
class HateXplainTask(Task):
    ...
    def __init__(self, 
        ...
        if demographic_field is None:
            raise ValueError("Must specify demographic field")
        self.demographic_column = demographic_field
        if self.demographic_column == "race":
            self.demographics = [
            "African",
            "Arab",
            "Asian",
        ...
        elif self.demographic_column == "gender":
        ...
        self.loss = torch.nn.CrossEntropyLoss(weight=torch.tensor([.8, .2], device=device))
        ...
        for (i, line) in enumerate(lines):
            examples.append(
                Example(
                    ...
                    # label=int(line['label']=='toxic'),
                    label=self.LABEL_TO_ID[line['label']],
                    demographics=line[self.demographic_column]
                )
            )
        return examples
